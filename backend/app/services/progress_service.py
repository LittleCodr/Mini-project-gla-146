...
def update_progress(db, user: dict, step_title: str) -> dict:
    ...
    user_id = user['id']
    progress_ref = db.collection('progress').document(f"{user_id}_{step_title.replace(' ', '_')}")
    ...
    logger.info(f"User {user_id} completed step: '{step_title}'")
    return progress
...
def get_progress(db, user: dict) -> schemas.ProgressResponse:
    ...
    # Approximate total steps across 10 domains × 10 steps
    TOTAL_STEPS = 100
